...
class MyNotes {
  ...
  async deleteNote(e) {
    ...
    try {
      ...
      if (response.data.userNoteCount < 3) {
  ...
  async createNote() {
    if (document.querySelector(".new-note-title").value === "" || document.querySelector(".new-note-body").value === "") {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(universityData.root_url + "/wp-json/wp/v2/note/", newNoteData);
      const arrText = ["You have reached your note limit.", "\n  You have reached your note limit.", "\r\n  You have reached your note limit."];
      if (arrText.includes(response.data)) {
        ...
        let newlyCreated = document.querySelector("#my-notes .fade-in-calc");
        setTimeout(function () {
...
class Search {
  ...
  async getResults() {
    try {
      if (this.searchField.value) {
        const respond = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(universityData.root_url + "/wp-json/university/v1/search?term=" + this.searchField.value);
